...
const parseData = (data) => {
  for(let i =0; i<props.allPolls.length; i++){
    ...
    arrResults.push( data.filter(element => element.pollid === props.allPolls[i].id) );
...
const barDisplays = () => {
  console.log("in barDisplays: ");
          return(arrResults.map( (array, index) => {
            return(
                <>
                  <BarResult data={array} question={props.allPolls[index].question} responses={[props.allPolls[index].response1,props.allPolls[index].response2,props.allPolls[index].response3,props.allPolls[index].response4]} />
                </>
            )
          }))
}
  ...
  return (
    <div>
      ...
      {localStorage.role === "admin" ?
        <>
        <Button color="warning" onClick={display}>See All Results</Button>
        {barDisplays()}
